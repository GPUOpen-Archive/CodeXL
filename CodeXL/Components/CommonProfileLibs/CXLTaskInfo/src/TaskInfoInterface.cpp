...
#if AMDT_ADDRESS_SPACE_TYPE == AMDT_64_BIT_ADDRESS_SPACE
#define AMDTPROCESSENUM_NAME AMDTPROCESSENUM64_NAME
#else
#define AMDTPROCESSENUM_NAME AMDTPROCESSENUM32_NAME
#endif
...
{
    wchar_t pidDirName[OS_MAX_PATH] = { 0 };
    ...
    /* [in] */  unsigned namesize)
{
    ...
    if (nullptr == pModulename || nullptr == pModulesize || nullptr == pModuleStartAddr)
    {
        return S_FALSE;
    }
...
void fnGetCSSModules(unsigned int pid, CSSMODMAP* pModMap, BOOL* pIs32)
{
    ...
    ::GetTempPath(OS_MAX_PATH, tempPath);
    ::GetTempFileName(tempPath, L"64S", 0, snapShotFile);

    wchar_t exePath[OS_MAX_PATH + 1] = { 0 };
    //Get the install path to /bin from the registry
    wchar_t BinFileName[OS_MAX_PATH + 1] = { 0 };

    // find the current executable file
    if (GetModuleFileName(nullptr, BinFileName, sizeof(BinFileName)))
    {
        wchar_t drive[_MAX_DRIVE];
        wchar_t dir[_MAX_DIR];

        // split the current executable file name into drive, path etc.
        _wsplitpath(BinFileName, drive, dir, nullptr, nullptr);
        wcscpy(exePath, L"\"");
        wcscat(exePath, drive);
        wcscat(exePath, dir);
    }

    wcscat_s(exePath, AMDTPROCESSENUM_NAME L".exe\" ");
    ...
    //launch the helper process to enum modules and dump the info to a file
