...
void vulkanFunctionDefs::AddInterfaceFunction(const std::string& apiStr, VkFuncId functionType, vkAPIType apiType)
{
    ...
    assert(m_sVKAPIMap.find(apiStr) == m_sVKAPIMap.end());
    assert(m_sVKAPIStringsMap.find(functionType) == m_sVKAPIStringsMap.end());
    assert(m_sVKAPITypeMap.find(apiStr) == m_sVKAPITypeMap.end());
