...
TraceTableItem* TraceTableModel::CloseLastOpenedPerfMarker(acTimelineItem* pTimelineItem)
{
    ...
    Marker marker;
    marker.pPerfMarkerItem = pOpenedItem;

    if (!m_openedPerfMarkerItemsStack.empty())
    {
        marker.pParentMarker = m_openedPerfMarkerItemsStack.top();
    }
...
{
    if (pFirst->itemType == API && pSecond->itemType == PERFMARKER)
    {
        // API - PERFMARKER
    ...
    if (pFirst->itemType == PERFMARKER && pSecond->itemType == API)
    {
        // PERFMARKER - API
    ...
    if (pFirst->itemType == PERFMARKER && pSecond->itemType == PERFMARKER)
    {
...
bool TraceTableModel::InitializeModel()
{
    ...
    GT_IF_WITH_ASSERT(m_pRootItem != nullptr)
    {
        ...
        while (listIndx < leftTraceTableItemList.size())
        {
            ...
            TraceTableItem* pParent = GetNextItemParent(pNextItemToAdd, pNextItemToAdd->itemType);
...
TraceTableItem* TraceTableModel::GetNextItemParent(const TraceTableItem* pNextItemToAdd, const TraceTableItemType& itemType) const
{
    ...
    GT_IF_WITH_ASSERT(pNextItemToAdd != nullptr)
    {
        ...
        else
        {
            ...
            {
                if (pNextItemToAdd == it->pPerfMarkerItem && nullptr != it->pParentMarker)
                {
                    pParent = it->pParentMarker;
                }
            }
        }
    }
