...
void kaApplicationCommands::ActivateMDITreeItem(const osFilePath& filePath)
{
    if (m_MDITreeItemfilePath != filePath)
    {
        ...
        GT_IF_WITH_ASSERT(nullptr != pKATreeHandler)
        {
            ...
            {
                //afApplicationTreeItemData* pFileItemData = FindFileItemData(filePathToActivate);
                kaProgram* pLastBuildProgram = KA_PROJECT_DATA_MGR_INSTANCE.GetLastBuildProgram();
                if (nullptr != pLastBuildProgram)
                {
                    // check if last build program contains this file
                    if (pLastBuildProgram->HasFile(filePathToActivate, AF_TREE_ITEM_ITEM_NONE))// filePathToActivate)
                    {

                        afApplicationTree* pTree = afApplicationCommands::instance()->applicationTree();
                        if (nullptr != pTree)
                        {
                            QTreeWidgetItem* pProgramItem = pKATreeHandler->GetProgramTreeItem(pLastBuildProgram);
