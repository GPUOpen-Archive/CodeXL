...
bool osInputFileImpl::open(const osFilePath& path, osChannel::osChannelType fileType)
{
    ...
    // Open the file:
    _pInputFileStream = fopen(path.asString().asUTF8CharArray(), fileOpenFlags.asASCIICharArray());

    if (fileType == osFile::OS_UNICODE_TEXT_CHANNEL)
    {
        // Read the unicode BOM:
        gtByte unicodeBom[2];
        gtSize_t amountOfRead = 0;
        bool rc = read(unicodeBom, 2, amountOfRead);
        GT_IF_WITH_ASSERT(rc)
        {
            retVal = ((unicodeBom[0] == (gtByte)0xff) && (unicodeBom[1] == (gtByte)0xfe));
        }
    }
    else
    {
        ...
        if (isOK())
        {
            ...
            while (goOn)
            {
                ...
                for (int j = 0; j < OS_FILE_CHUNK_READ_SIZE; j++)
                {
                    ...
                    size_t bytesRead = fread(&pDataBuffer[i], 2, 2, _pInputFileStream);
                    (void)bytesRead;
...
{
    (void)(seekStartPosition); // unused
    bool retVal = false;

    if (_pInputFileStream != NULL)
    {
        int rc = fseek(_pInputFileStream, offset, 0);
        retVal = (rc == 0);
    }
