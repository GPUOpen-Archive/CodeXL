...
parse_floating_number(const char* first, const char* last, C& db)
{
    ...
    if (static_cast<std::size_t>(last - first) > N)
    {
        ...
        if (*t == 'E')
        {
            ...
            int n = _snprintf(num, sizeof(num), float_data<Float>::spec, value);
        ...
        if (*first == 'S')
        {
            switch (first[1])
            {
                ...
                case '_':
                    if (!db.subs.empty())
                    {
                        const auto& sb = db.subs.front();

                        for (auto n = sb.begin(), nEnd = sb.end(); n != nEnd; ++n)
                        {
...
parse_template_param(const char* first, const char* last, C& db)
{
    if (last - first >= 2)
    {
        if (*first == 'T')
        {
            if (first[1] == '_')
            {
                if (!db.template_param.empty() && !db.template_param.back().empty())
                {
                    auto& st = db.template_param.back().front();

                    for (auto it = st.begin(), itEnd = st.end(); it != itEnd; ++it)
                    {
                        db.names.push_back(*it);
                    }
            ...
            else if (isdigit(first[1]))
            {
                ...
                if (!db.template_param.empty() && sub < db.template_param.back().size())
                {
                    const auto& sb = db.template_param.back()[sub];

                    for (auto temp = sb.begin(), tempEnd = sb.end(); temp != tempEnd; ++temp)
                    {
...
parse_unresolved_name(const char* first, const char* last, C& db)
{
    if (last - first > 2)
    {
        ...
        else if (last - t > 2 && t[0] == 's' && t[1] == 'r' && !db.names.empty())
        {
            if (t[2] == 'N')
            {
                t += 3;
    ...
    if (last - first >= 4)
    {
        ...
        if (t[0] == 'n' && (t[1] == 'w' || t[1] == 'a'))
        {
            ...
            if (!db.names.empty())
            {
                ...
                if (has_expr_list && !db.names.empty())
                {
    ...
    if (last - first >= 3 && first[0] == 'c' && first[1] == 'v')
    {
        ...
        if (t != first + 2 && t != last)
        {
            ...
            else
            {
                ...
                if (*t == 'E')
                {
                    db.names.emplace_back(string_pair());
                }
                else
                {
                    bool first_expr = true;
        ...
        while (*t0 != 'E')
        {
            ...
            switch (*t0)
            {
                case 'S':
                    ...
                    if (t1 != t0 && t1 != last && !db.names.empty())
                    {
                        auto name = db.names.back().move_full();
                        db.names.pop_back();
            ...
            switch (*t)
            {
                ...
                case 'd':
                    if (++t != last)
                    {
                        ...
                        if (t1 != last && *t1 == '_' && !db.names.empty())
                        {
                            t = t1 + 1;
    ...
    if (last - first >= 2)
    {
        ...
        switch (*t0)
        {
            ...
            default:
            {
                ...
                if (t1 != t0)
                {
                    if (t1 != last && *t1 == 'I' && !db.names.empty())  // <unscoped-template-name> <template-args>
                    {
                        db.subs.push_back(typename C::sub_type(1, db.names.back(), db.names.get_allocator()));
                        t0 = t1;
                        t1 = parse_template_args(t0, last, db);
    ...
    if (first != last)
    {
        switch (*first)
        {
            ...
            default:
            {
                ...
                if (t != first)
                {
                    if (t != last && *t != 'E' && *t != '.' && !db.names.empty())
                    {
...
{
    if (first != last && *first == '.' && !db.names.empty())
    {
...
public:
    ...
    void construct(pointer _Ptr, T&& _Val)
    {
        ...
        ::new((void*)_Ptr) T(_STD forward<T>(_Val));
    }
    ...
    {
        // construct object at _Ptr with value _Val
        ::new((void*)_Ptr) T(_STD forward<_Other>(_Val));
    }
...
typedef std::basic_string<char, std::char_traits<char>, malloc_alloc<char> > String;
...
{
    if (mangled_name == nullptr || buf == nullptr || n == 0)
    {
        if (status)
        {
            *status = invalid_args;
    ...
    size_t internal_size = n;
    arena<bs> a;
    struct Db
    {
        typedef String String;
        typedef std::vector<string_pair, short_alloc<string_pair, bs> > sub_type;
        typedef std::vector<sub_type, short_alloc<sub_type, bs> > template_param_type;
        sub_type names;
        template_param_type subs;
        std::vector<template_param_type, short_alloc<template_param_type, bs> > template_param;
        unsigned cv;
        unsigned ref;
        ...
        Db(arena<bs>& ar) :
            names(short_alloc<string_pair, bs>(ar)),
            subs(0, names, short_alloc<sub_type, bs>(ar)),
            template_param(0, subs, short_alloc<template_param_type, bs>(ar))
        {}
    };
    Db db(a);
    db.cv = 0;
    ...
    if (internal_status == success && !db.names.empty())
    {
        sz = db.names.back().size();

        if ((sz + 1) <= internal_size)
        {
            db.names.back().first += db.names.back().second;
            std::memcpy(buf, db.names.back().first.data(), sz);
            buf[sz] = char(0);
        }
    }
    else
...
{
    unsigned len = abi::__cxa_demangle(pMangledName, pDemangledName, static_cast<size_t>(maxDemangledNameLen), NULL);

    if (0U == len)
    {
        len = DemangleExternCNameVS(pMangledName, pDemangledName, maxDemangledNameLen);
    }
...
wchar_t* SymbolEngine::DemangleNameIA(const char* pMangledName)
{
    ...
    if (0U != DemangleNameIA(pMangledName, demangledNameBuffer, 2048))
    {
    ...
    return AnsiToWide(pDemangledNameAnsi);
}
